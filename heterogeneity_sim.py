...
### --- Global parameters ---
past_alpha_true, past_beta_true = 2.0, 10.0
curr_alpha_true, curr_beta_true = 3.0, 18.0
R = 50000 # replicates
# Note: It takes about 5 minutes to run 1k replicates

K = 10 # number of trials in past data
N_curr = 20 # number of current sites
global_t = 10 # global census time
weights = [0, 0.25, 0.5, 0.75, 1] # weight on past trials data

# File Saving Setup
DATA_DIR = Path("data/heterogeneity")
DATA_DIR.mkdir(parents=True, exist_ok=True)

FIGURES_DIR = Path("figures/heterogeneity")
FIGURES_DIR.mkdir(parents=True, exist_ok=True) 
print(f"Saving figures to: {FIGURES_DIR.resolve()}")

RESULTS_DIR = Path("results/heterogeneity")
...
for i in range(R): # for each replicate
    if (i+1) % 2000 == 0: # for the sake of sanity :) no more waiting in vain
    ...
    # Past trials data
    sim_N_n, t_obs= simulate_summary_data(past_alpha_true, past_beta_true, 
                                         psi=1/50, gamma=5, 
    ...
    # Current trial data
    trial = SingleRegionRecruitment(1, N_curr, t=global_t, alpha=curr_alpha_true, beta=curr_beta_true)
    trial.genData()
...
try:
    alpha_file_exists = ALPHA_RESULTS_FILE.exists()
    alpha_df.to_csv(
        ALPHA_RESULTS_FILE, 
        mode='a' if alpha_file_exists else 'w',
        header=not alpha_file_exists,
        index=False
    )

    beta_file_exists = BETA_RESULTS_FILE.exists()
    beta_df.to_csv(
        BETA_RESULTS_FILE, 
        mode='a' if beta_file_exists else 'w',
        header=not beta_file_exists,
        index=False
    )
...
print("\n--- Quantitative Metrics (True Alpha=2.0, True Beta=10.0) ---")
...
for w_key in all_keys_for_table:
    ...
    a_bias = np.mean(a_hats) - curr_alpha_true
    b_bias = np.mean(b_hats) - curr_beta_true
    
    a_mse = np.mean((a_hats - curr_alpha_true)**2)
    b_mse = np.mean((b_hats - curr_beta_true)**2)
