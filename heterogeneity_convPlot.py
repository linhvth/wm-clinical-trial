...
try:
    df_alpha_full = pd.read_csv('results/heterogeneity/251025_50k_alpha_estimates.csv')
    df_beta_full = pd.read_csv('results/heterogeneity/251025_50k_beta_estimates.csv')
except FileNotFoundError:
    print("WARNING: Could not find files, trying 'alpha_estimates.csv'")
    # Fallback to the names from the previous script
    df_alpha_full = pd.read_csv(Path('data/heterogeneity') / 'alpha_estimates.csv')
    df_beta_full = pd.read_csv(Path('data/heterogeneity') / 'beta_estimates.csv')
...
all_running_metrics_filtered = all_running_metrics[all_running_metrics['weight'] != '0.0']

g = sns.relplot(
    data=all_running_metrics, # or all_running_metrics_filtered
    x='Replicates',
    y='Value',
    hue='weight',
    col='Metric',
...
plot_path = FIGURES_DIR / "metrics_convergence_trace.png"
